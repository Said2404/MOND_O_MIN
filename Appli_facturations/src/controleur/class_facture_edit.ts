...
type TFactureEditForm = {
  ...
  edtCom: HTMLInputElement;
  edtNoClient: HTMLInputElement;
  listeLivraison: HTMLSelectElement;
  edtRemise: HTMLInputElement;
  btnRetour: HTMLInputElement;
  btnValider: HTMLInputElement;
  btnAnnuler: HTMLInputElement;
  lblDetailsClient: HTMLLabelElement;
  lblDetailsLivraison: HTMLLabelElement;
  lblDateErreur: HTMLLabelElement;
  lblClientErreur: HTMLLabelElement;
  lblRemiseErreur: HTMLLabelElement;
  lblProdErreur: HTMLLabelElement;
  ...
  lblDetailsProduit: HTMLLabelElement;
...
class VueFactureEdit {
  ...
  private _grille: TProduitsByFacture; // tableau des produits de la facture
  private _erreur: {
    // tableau contenant les messages d'erreur pour chaque type d'erreur pour chaque zone de saisie à vérifier
    [key: string]: TErreur;
  };
  private _detailsProduitSelectionne: string;
  ...
  get grille(): TProduitsByFacture {
    return this._grille;
  }
  get erreur(): { [key: string]: TErreur } {
    return this._erreur;
  }
  get detailsProduitSelectionne(): string {
    return this._detailsProduitSelectionne;
  }
  set detailsProduitSelectionne(string: string) {
  ...
  init(form: TFactureEditForm): void {
    ...
    this.form.edtNoClient.onchange = function (): void {
      vueFactureEdit.detailClient(String(vueFactureEdit.form.edtNoClient.value));
    };
    this.form.listeLivraison.onchange = function (): void {
    ...
    this.form.btnAjouterProduit.onclick = function (): void {
      vueFactureEdit.ajouterProduitClick();
    };
    this.form.btnValiderProduit.onclick = function (): void {
      vueFactureEdit.validerProduitClick();
    };
    this.form.btnAnnulerProduit.onclick = function (): void {
  ...
  calculeMontants(valeur: string, client: string): void {
        ...
        if (leClient.idCli !== "") {
          ...
          if (Number(valeur) >= 0 && Number(valeur) <= 100 && Number(valeur) <= Number(leClient.tauxMaxRemiseCli)) {
            ...
            for (let id in this.grille) {
              const unProduitByFacture: UnProduitByFacture = this.grille[id];
  ...
  detailsProduitCliqué(codeProd: string): string {
  ...
  detailsProduitSurvolé(codeProd: string): void {
    const lesProduits = new LesProduits();
    const detailProd = this.form.lblDetailsProduit;
    const produit = lesProduits.byCodeProd(codeProd);
    if (produit.codeProd !== "") {
  ...
  verifNumCli(valeur: string): void {
    const lesFactures = new LesFactures();
  ...
  validerClick(): void {
    ...
    if (correct) {
      ...
      const lesProduitsByFacture: LesProduitsByFacture = new LesProduitsByFacture();
  ...
  modifierProduitClick(id: string): void {
    ...
    const lesProduits = new LesProduits();
    const unProduit: UnProduit = lesProduits.byCodeProd(id);
  ...
  validerProduitClick(): void {
    ...
    correct =
      this.traiteErreur(
        this._erreur.listeProduit,
        this.form.lblSelectProduitErreur
      ) && correct;
    correct =
    ...
    if (correct) {
      ...
      const unProduit: UnProduit = lesProduits.byCodeProd(
        this._form.listeProduit.value
      );
      const unProduitByFacture: UnProduitByFacture = new UnProduitByFacture(
        unProduit,
        parseInt(this._form.edtQte.value)
      );
